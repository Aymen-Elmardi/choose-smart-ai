      ...
      <div className="section-container max-w-3xl mx-auto">
        <div className={`text-center mb-12 reveal ${isInView ? "visible" : ""}`}>
          <p className="text-xs font-bold uppercase tracking-widest text-primary mb-3">
            Assessment Example Output
          </p>
          <p className="text-muted-foreground max-w-xl mx-auto leading-relaxed">
            Your risk profile determines who approves you, what rate you pay, and whether your account stays open. Most businesses never see it until something goes wrong.
          </p>
        </div>

        <div className={`reveal stagger-1 ${isInView ? "visible" : ""}`}>
          {/* Section 1 - Business Profile */}
          <div className="rounded-xl border-2 border-primary/30 bg-primary/5 p-6 mb-6 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg hover:border-primary/60">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              {[
                ...
                { icon: TrendingUp, label: "Monthly Volume", value: "$150k / mo" },
                { icon: CreditCard, label: "Avg. Transaction", value: "$95" },
                { icon: Globe, label: "Location", value: "United States" },
              ].map((item) => (
            ...
            <div className="mt-4 pt-4 border-t border-primary/15">
              ...
              <div className="flex flex-wrap gap-2">
                {["Low fees", "Recurring billing"].map((p) => (
          ...
          {/* Section 2 - Best Match */}
          <div className="rounded-xl border border-border/60 bg-card p-5 mb-5 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg hover:border-primary/50">
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle className="w-4 h-4 text-muted-foreground" />
              <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Best Match</span>
            </div>
          ...
          <div className="rounded-xl border-2 border-destructive/20 bg-destructive/5 p-5 mb-8">
            ...
            <div className="space-y-3">
              {[
                { name: "Square", reason: "No subscription billing at this volume" },
                { name: "SumUp", reason: "Volume cap below $150k/month threshold" },
              ].map((item) => (
